...
- Costruire un assistente vocale completamente locale basato su OpenClaw, in tre tier di complessità crescente
...
- Portare HomeClaw in ogni stanza con satelliti economici (ESP32-S3, Raspberry Pi Zero 2 W)
...
```
                           ...
                           │                                 │
   ┌──────────────┐  mic   │  ┌───────────────────────────┐  │
   │ ReSpeaker    │───────▶│  │ wyoming-satellite :10700  │  │
   │ USB 4-Mic    │        │  └────────────┬──────────────┘  │
   │ + altoparl.  │◀───────│               │ Wyoming TCP     │
   │              │  audio │               ▼                 │
   └──────────────┘        │  ┌───────────────────────────┐  │
                           │  │  homeclaw-bridge.py       │  │
                           │  │  (orchestratore)          │  │
        ...
        [luci, prese, Sonos, termostato, ...]
                ...
                Tier C: SATELLITI IN ALTRE STANZE (multi-stanza)
                
        ┌───────────────────┐         ┌───────────────────┐
        │ ESP32-S3-BOX-3    │         │ Pi Zero 2 W       │
        │ cucina            │         │ camera da letto   │
        │ microWakeWord     │ ─Wi-Fi─▶│ wyoming-satellite │ ─▶ HUB
...
| Componente | Modello | Prezzo indicativo | Note |
|---|---|---|---|
| Single board computer | Raspberry Pi 5 16GB | €95 | Il 16GB è fondamentale se vuoi far girare un LLM locale; 8GB ok solo se deleghi sempre al cloud |
| Acceleratore AI | Raspberry Pi AI HAT+ 2 (Hailo-10H) | €110 | Opzionale ma consigliato; senza, Whisper "small" impiega 2+ secondi |
...
```
t=0 ms       Microfono cattura audio PCM 16 kHz mono in streaming continuo
t=+200 ms    wyoming-openwakeword rileva "Ehi Claw"
t=+220 ms    LED blu pulsante; wyoming-satellite registra il comando
t=+2200 ms   Silenzio rilevato da VAD; il comando è "che tempo farà domani a Rimini"
t=+2250 ms   Audio inoltrato a wyoming-faster-whisper (small-int8 su Hailo)
t=+2850 ms   Trascrizione pronta; homeclaw-bridge riceve il testo
t=+2900 ms   Testo inoltrato all'agente HomeClaw sul canale 'voice'
t=+3500 ms   Agente decide: query meteo → skill weather('Rimini', 'tomorrow')
t=+3550 ms   Skill weather risponde: "Domani sereno, massima 22 gradi"
t=+3600 ms   homeclaw-bridge invia la risposta a wyoming-piper
t=+3900 ms   Piper ha sintetizzato i 300 ms di audio iniziali
...
Segui la guida ufficiale su `raspberrypi.com/documentation/computers/ai.html` — cambia di mese in mese, non ha senso incollare qui comandi che diventerebbero stantii. Alla fine deve funzionare:
...
| Modello | Senza NPU | Con Hailo-10H | Accuracy italiana | RAM |
|---|---|---|---|---|
| `tiny-int8` | 0,4 s | n/d | 62% | 150 MB |
| `base-int8` | 1,1 s | 0,5 s | 78% | 270 MB |
| `small-int8` | 2,3 s | 0,7 s | 89% | 870 MB |
| `medium-int8` | 5,8 s | 1,4 s | 94% | 2,4 GB |
...
Provale con `echo "Ciao, sono HomeClaw" | piper --model it_IT-paola-medium.onnx --output_file test.wav && aplay test.wav` prima di integrarle.
...
# - nome dell'agente: HomeClaw
# - modello: Claude Sonnet 4.6 (fallback cloud) + Nemotron-3B via Hailo (primario locale)
# - canale: saltare Telegram per ora, useremo il bridge custom
```

### Step 7. Installare la skill `homeclaw-bridge`

Clona il repo di questo capitolo:

```bash
cd ~
git clone https://github.com/tuo-account/homeclaw-repo
cd homeclaw-repo
...
async def _on_transcript(self, transcript: Transcript) -> None:
    ...
    synth = Synthesize(text=clean_text, voice={"name": self._tts_voice})
...
1. Apri il Colab notebook: `colab.research.google.com/github/dscripka/openWakeWord/blob/main/notebooks/automatic_model_training_simple.ipynb`
...
async def light_control(area: str, action: str, brightness: int = None):
    """Control a Home Assistant light or group via REST API."""
    service = "turn_on" if action == "on" else "turn_off"
    body = {"area_id": area}
    if brightness is not None:
        body["brightness_pct"] = brightness
    return await _ha_post(f"/api/services/light/{service}", body)
```
...
| Setup | Hardware | Servizi/anno | Totale 3 anni | Privacy | Estendibilità |
|---|---|---|---|---|---|
| Alexa Echo Dot gen 5 | €55 | €0–60 (Prime/Alexa+) | €55–235 | Cloud USA | Bassa |
| Google Nest Mini | €60 | €0 ma pubblicità in arrivo | ~€60 | Cloud USA | Media |
| HomeClaw Tier A | €130 | €30–60 (API + elettricità) | €220–310 | Locale ibrido | Alta |
| HomeClaw Tier B | €380 | €30–120 (API + elettricità) | €470–740 | Locale ibrido | Altissima |
| HomeClaw Tier B air-gapped | €380 | €12 (elettricità) | €416 | Totale | Altissima |
| HomeClaw Tier C 3 stanze | €550 | €50–150 | €700–1000 | Locale ibrido | Altissima |
...
## Errori comuni — albero di diagnosi

Quando qualcosa non funziona, segui questo albero. Porta al problema nel 90% dei casi.

```
L'agente non risponde per niente?
├── Il LED si accende quando dici "Ehi Claw"?
│   ├── NO → wake word non rileva
│   │       → verifica mic: arecord -D plughw:... test.wav && aplay test.wav
│   │       → verifica wyoming-openwakeword: systemctl status
│   │       → controlla threshold: prova --threshold 0.4 (più sensibile)
│   │       → fine-tune il modello se wake word custom
│   └── SÌ → il LED pulsa blu ma non arriva risposta
...
│   ├── Su Tier B con NPU? → hailortcli fw-control identify; se N/A, Hailo driver broken
│   │                        → apt-mark hold raspberrypi-kernel, reinstalla driver Hailo
│   ├── RAM esaurita? → free -h; se swap in uso, passa a Pi5 16GB o disabilita modelli grossi
│   └── Rete Wi-Fi? → iperf3 all'hub: se <50 Mbps o instabile, switch a Ethernet
│
├── Wake word si attiva da sola (falsi positivi)?
│   ├── Più di 2/ora? → fine-tune con registrazioni reali (repo docs/wakeword-training.md)
│   ├── Solo con TV accesa? → aumenta threshold a 0.6, o abilita VAD più stretto
│   └── Solo con certe voci? → aggiungi sample negative di quelle voci al training
│
├── Audio distorto o rotto?
│   ├── Alimentatore da 27W ufficiale? (15W = throttling USB)
│   ├── ReSpeaker riconosciuto? arecord -L mostra ArrayUAC10?
│   ├── Bluetooth che si scollega? disable Bluetooth power-save (vedi Errori Comuni più sotto)
│   └── HDMI audio interferisce? in /boot/firmware/config.txt: dtparam=audio=off
│
└── Agente risponde con Markdown che il TTS legge letter-by-letter?
    ├── Prima: aggiorna SOUL.md con esempi concreti ("non dire *importante* ma 'importante'")
    └── Poi: verifica che homeclaw-bridge fa _clean_for_tts() sulla risposta
```

**Altri errori frequenti non nell'albero:**

**Il driver Hailo crasha dopo `apt full-upgrade`.** Soluzione: `sudo apt-mark hold raspberrypi-kernel raspberrypi-kernel-headers` finché Hailo non rilascia driver per il kernel aggiornato.
...
├── led-feedback/
│   ├── homeclaw_led_feedback.py       # Script feedback LED APA102
│   ├── requirements.txt
│   └── README.md
